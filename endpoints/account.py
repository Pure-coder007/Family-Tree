...
def create_fam_member():
    try:
        ...
        if not valid:
            return return_response(
                HttpStatus.BAD_REQUEST, status=StatusRes.FAILED, message=message
            )
        ...
        if err:
            return return_response(
                HttpStatus.INTERNAL_SERVER_ERROR, status=StatusRes.FAILED, message=err
            )
    ...
    try:
        ...
        if not fam_event_year:
            return return_response(
                HttpStatus.BAD_REQUEST,
                status=StatusRes.FAILED,
                message="Event year is required",
            )
...
@account.route(
    f"{ACCOUNT_URL_PREFIX}/delete_gallery_item/<gallery_id>", methods=["DELETE"]
)
@jwt_required()
def delete_gallery(gallery_id):
    try:
        ...
        if success:
            return return_response(
                HttpStatus.OK, status=StatusRes.SUCCESS, message="Gallery item deleted"
            )
        else:
            return return_response(
                HttpStatus.NOT_FOUND,
...
def add_to_logo():
    try:
        if request.method == "GET":
            ...
            return return_response(HttpStatus.OK, status=StatusRes.SUCCESS, **data)
